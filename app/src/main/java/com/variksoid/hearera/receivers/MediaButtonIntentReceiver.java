package com.variksoid.hearera.receivers;
...
public class MediaButtonIntentReceiver extends BroadcastReceiver {
        ...
        public void handleMessage(final Message msg) {
            if (msg.what == MSG_HEADSET_DOUBLE_CLICK_TIMEOUT) {
                ...
                if (command != null) {
                    ...
                    // Если вызывается OnStartCommand, воспроизведение запускается автоматически
    ...
    public static boolean handleIntent(final Context context, final Intent intent) {
        ...
        if (Intent.ACTION_MEDIA_BUTTON.equals(intentAction)) {
            ...
            // Возврат к системному времени, если время события недоступно.
            ...
            if (command != null) {
                if (action == KeyEvent.ACTION_DOWN) {
                    if (event.getRepeatCount() == 0) {
                        // Учитывать только первое событие в последовательности, а не повторяющиеся события,
                        // чтобы мы не срабатывали в тех случаях, когда первое событие ушло на
                        // другое приложение (например, когда пользователь завершает телефонный звонок,
                        // долгое нажатие на кнопку гарнитуры)
                        // Служба может быть запущена, а может и не запущена, но нам нужно отправить
                        // команду.
    ...
    private static void acquireWakeLockAndSendMessage(Context context, Message msg, long delay) {
        if (mWakeLock == null) {
            ...
            assert pm != null;
            mWakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "hearera:MediaButtonWakeLock");
            mWakeLock.setReferenceCounted(false);
        }
        // Установите частичную блокировку пробуждения на 5 минут после отправки команды кнопки мультимедиа.
