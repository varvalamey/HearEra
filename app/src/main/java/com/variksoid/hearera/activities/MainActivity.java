...
public class MainActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, PlayStatusChangeListener, SynchronizationStateListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mListView.setOnItemClickListener((adapterView, view, i, rowId) -> {
            // Открыть AlbumActivity при нажатии на альбом
        ...
        mListView.setMultiChoiceModeListener(new AbsListView.MultiChoiceModeListener() {
            ...
            public boolean onActionItemClicked(ActionMode actionMode, MenuItem menuItem) {
                // взять ids треков для всех выбранных альбомов
                ...
                switch (menuItem.getItemId()) {
                    case R.id.menu_delete_from_db:
                        // Удалить выбранные альбомы из базы данных если они больше не существуют
                        deleteSelectedAlbumsFromDBWithConfirmation();
                        actionMode.finish();
                        return true;
                    case R.id.menu_mark_as_not_started:
                        // Отметить все треки в выбранном альбоме как не начатые
                    ...
                    case R.id.menu_mark_as_completed:
                        // Отметить все треки в отмеченном альбоме как прослушанные
                        for (AudioFile audioFile : albumAudioFiles) {
        ...
        // Установка SwipeRefreshLayout onRefresh action
        ...
        // Установка the FAB onClickListener
        ...
        // Установка play status receiver
        ...
        // Извлечь код версии и название версии приложения
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor cursor) {
        ...
        if (mPlayer != null) {
            ...
            // Установить время прослушивания альбома
            ...
            // Получить позицию воспроизводимого в данный момент элемента ListView
            int currPosition = -1;
    ...
    protected void onRestart() {
        // Перезапустить если тема была изменена
    ...
    /*
     * Привяжите MainActivity к MediaPlayerService, если служба была запущена в PlayActivity.
     */
    private final ServiceConnection serviceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            ...
            // Выполнить действия которые могут быть выполненны только если сервис подключен
            // Установка воспроизведение-пауза картинок FAB в соответсвии с данным состоянием MediaPlayerSercvice
    ...
    /*
     * Отвязать MainActivity от MediaPlayerService, когда пользователь удаляет уведомление
     */
    ...
    /*
     * Отвязать MainActivity от MediaPlayerService, но снова привязать при возврате к активности
     */
    ...
    /*
     * Обновить прогресс для альбома воспроизводимого в данный момент аудиофайла
     */
    private void setCompletedTimeUpdater() {
        mHandler = new Handler();
        mRunnable = () -> {
            // Остановить runnable, когда служба не привязана
            if (!mServiceBound) {
                mHandler.removeCallbacks(mRunnable);
                return;
            }
            ...
            if (mPlayer != null && mPlayer.isPlaying() && mPlayer.getCurrentAudioFile().getID() == mCurrUpdatedAudioId) {
                // Установите строку прогресса для альбома воспроизводимого в данный момент аудиофайла.
    ...
    /*
     * Показать селектор файлов, где пользователь может выбрать каталог для экспорта базы данных
     */
    private void showExportDirectorySelector() {
        // Позвольте пользователю выбрать каталог, в котором будет сохранена база данных.
    ...
    /*
     * Показать селектор файлов, где пользователь может выбрать файл .db, из которого нужно импортировать базу данных.
     */
    ...
    /*
     * Показать диалоговое окно подтверждения и позволить пользователю решить, удалять ли выбранные альбомы
     * и/или треки из базы данных
     */
    private void deleteSelectedAlbumsFromDBWithConfirmation() {
        ...
        builder.setPositiveButton(R.string.dialog_msg_ok, (dialog, id) -> {
            // Пользователь нажал кнопку «Ок», поэтому удалить альбом и/или треки из базы данных
            int deletionCount = 0;
    ...
    /*
     * Показать диалоговое окно подтверждения и позволить пользователю решить, удалять ли выбранные альбомы
     * и/или его треки из базы данных
     */
    private void deleteSelectedAlbumWithConfirmation(final ArrayList<AudioFile> selectedTracks) {
        ...
        builder.setPositiveButton(R.string.dialog_msg_ok, (dialog, id) -> {
            // Удалить треки в выбранных альбомах
            int trackDeletionCount = 0;
            for (AudioFile audioFile : selectedTracks) {
                long audioFileID = audioFile.getID();

                // Остановите MediaPlayerService, если текущий воспроизводимый файл находится в удаленном каталоге.
                if (mPlayer != null) {
            ...
            // Обновить таблицы базы данных и уведомить пользователя об удалении
            mSynchronizer.updateDBTables();
